...
from task1 import WordPieceTokenizer  # Taking Task 1 
from task2 import Word2VecModel      # For loading Task 2 


# NeuralLMDataset for the start
...
# NeuralLM1: Baseline with a single hidden layer and ReLU activation ( Simple starter ).
class NeuralLM1(nn.Module):
    ...
    def forward(self, context):
        embeds = self.embedding(context)  # context here is the following = [batch_size, context_size, embedding_dim]
        embeds = embeds.view(embeds.size(0), -1)  # Reshape to [batch_size, context_size * embedding_dim]
        out = self.fc1(embeds)
        out = self.relu(out)
        logits = self.fc2(out)
        return logits

# NeuralLM2: Deeper network with two hidden layers and dropout ( Same thing but two layer and dropout).
...
# NeuralLM3: Uses LeakyReLU, an extra hidden layer, and a larger context window (3 layers and leaky relu).
...
# Training and Evaluating the models and the helper functions for it
...
def plot_history(histories, model_names):
    """Plots training and validation loss for multiple models in 1 single plot."""
...
def predict_next_tokens(model, tokenizer, vocab, sentence, num_tokens=3, device='cpu'):
    ...
    # Use the correct context size from the model.
    context_size = model.context_size if hasattr(model, 'context_size') else 4

    # For prediction, if we have fewer than context_size tokens, pad with "[PAD]".
...
def main():
    ...
    # Split into training and validation (80-20 split as asked)
    ...
    vocab_file = "vocabulary_88.txt"  # The vocab file created before
    with open(vocab_file, "r", encoding="utf-8") as f:
        vocab = [line.strip() for line in f.readlines()]
    vocab_size = len(vocab)
    print(f"Vocabulary size: {vocab_size}")
    
    embedding_dim = 50  # Same dimension as in Task 2
    context_size = 4  
